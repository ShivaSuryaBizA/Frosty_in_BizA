...
QUALIFIED_TABLE_NAME = "COVID19_DEMO.PUBLIC.CN_BTC"
METADATA_QUERY = "SELECT COUNTRY_REGION, CNT FROM COVID19_DEMO.PUBLIC.CN_BTC;"
TABLE_DESCRIPTION = """
...
<columns>\n\n{columns}\n\n</columns>
    ...
    if metadata_query:
        ...
        metadata = "\n".join(
            [
                f"- **{metadata['COUNTRY_REGION'][i]}**: {metadata['CNT'][i]}"
                for i in range(len(metadata["COUNTRY_REGION"]))
            ]
        )
        context = context + f"\n\nAvailable variables by COUNTRY_REGION:\n\n{metadata}"
    return context
